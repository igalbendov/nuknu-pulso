...
// Dispara un aviso a todo el equipo (menos al autor) vía la función serverless.
export async function notifyTeam({ title, body, url, excludeName }) {
  try {
    await fetch("/api/notify", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title, body, url: url || "/", excludeName }),
    });
  } catch (e) {}
}
